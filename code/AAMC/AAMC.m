...
% 2) Correcting bug in the changes_count(node,t) calculation;
% must be refreshed within the i-node loop
% 3) Correcting calculation of metric7 with changes_count(node,t);
% the way it is, it is always zero!
%%%%%%%%%%%%%%%%%%
% 30.05.13 23:41
% 1) Corrected calculation of changes_count(node,time)
...
for iindexxx = 1:N;
    ...
    if randostate(iindexxx,jjindexxx) <= 0.0833 % so that (length(STATES)*randostate(node)) >= 0.5
        randostate(iindexxx,jjindexxx) = 0.09;
    end
    end % jjndexxx
end % iindexxx
...
%           changes_count(i,t) = changes_count(i,t) + 1;
...
ER = sum(ERRRATEI,1)/iterations;
    ...
    disp(downratio*100);     
     ...
     disp('###############')
    disp('adaptive scheme, METRIC = duplicates - NEC - mode switch count, AMC with policy choosing, dynamic mode & beta');
    disp('**************');        
